...
export default function EventCreate() {
  ...
  async function handleSubmit(e) {
    ...
    if (!formData.title.trim()) { setErrorMsg("El títol és obligatori"); setIsSubmitting(false); return; }
    if (!formData.description.trim()) { setErrorMsg("La descripció és obligatòria"); setIsSubmitting(false); return; }
    if (!formData.category) { setErrorMsg("Selecciona una categoria"); setIsSubmitting(false); return; }
    if (!formData.date) { setErrorMsg("Indica una data i hora"); setIsSubmitting(false); return; }
    if (!formData.lat || !formData.lng) { setErrorMsg("Falta la ubicació del mapa"); setIsSubmitting(false); return; }
  ...
  return (
    <div className="p-6 max-w-xl mx-auto bg-white shadow-md rounded-2xl mt-10">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Crear esdeveniment</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1">
            <label className="text-sm font-semibold text-gray-600">Descripció</label>
            <textarea
            name="description"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
                <label className="text-sm font-semibold text-gray-600">Data i Hora</label>
                <input
                type="datetime-local"
                name="date"
                value={formData.date}
        ...
        <div className="flex flex-col gap-1">
            <label className="text-sm font-semibold text-gray-600">Imatge de portada</label>
            <input 
            type="file"
            accept="image/*"
